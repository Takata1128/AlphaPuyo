...
mcts::Node::Node(State state, int d, float p, float r, int turn) {
    this->state = state;
    this->p = p;
    this->d = d;
    this->r = r;
...
float mcts::Node::evaluate() {
    if(state.isDone() || turn == MAX_STEP) {
        float value = (state.isLose() ? -1 : r);
        w += value;
        n++;
        return value;
    }
    ...
    if(this->childNodes.empty()) {
        ...
        float value = this->nextChildNode().evaluate();
        this->w += value;
        this->n++;
        return value;
    }
...
std::vector<float> mcts::mctsScores(VVI field, VVI next2, int turn,
                                    float temperature) {}
